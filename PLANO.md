...
| D28 | O ato no sítio do órgão de chegada | Fase 2 (v2) | **quarta e última** tentativa de destino, por busca web com chave (serper.dev; 2.500 consultas grátis e ~30 por mês de consumo). Alcança o que nem o DOU, nem o ranking, nem o diário municipal alcançam — tribunal de contas estadual, defensoria, TRT —, porque esses órgãos publicam em sítio próprio. O órgão vem do **domínio**, nunca do texto, o que dispensa IA. **Etapa OPCIONAL:** sem chave devolve `None` e nada acontece. Revisa a **D27** num ponto: **convocação não publica sozinha** — a D27 tinha publicado "Prefeitura de Paulínia" para LEONARDO TOIOMOTO com base em três convocações, e o Diário da Cidade de São Paulo traz "NOMEAR LEONARDO TOIOMOTO", TCM-SP. Pela terceira vez fica medido que cruzar com o ranking é o **portão errado**, e por pedido do usuário o cruzamento virou **selo**: `RANKING` quando o órgão também consta da ficha, `GOOGLE` quando só a busca o conhece |

| D29 | O site lê arquivo final, não monta dado | Fase 3 | **a mescla sai do navegador e vira etapa de publicação.** `scripts/publicacao.py` (biblioteca pura) + `gerar_publicacao.py` (executável) aplicam ao `dados.csv` as saídas que só o DOU conhece (D22) e os destinos do ranking (D24), e gravam **`data/painel.csv`** e **`public/alteracoes.json`** — os arquivos que as três páginas leem. As saídas **não vão para o Git**: nascem no `deploy-pages.yml` (via `prebuild`), que já dispara a cada push e ao fim da varredura do DOU, então o frescor é o mesmo de antes e não há 425 KB reescritos por dia. Isso é possível porque as quatro entradas da mescla **estão todas versionadas** — o que impedia era o `construir_painel.py` depender dos snapshots do Portal, e a mescla não depende. Custos aceitos pelo usuário: o front passa a exigir Python para rodar localmente, e o dado publicado deixa de ser inspecionável por `git diff`. O porte foi **provado contra a implementação antiga**: o TypeScript compilado com esbuild e o Python produziram as mesmas 2.009 linhas × 35 colunas e o mesmo log de 729 mudanças em 48 competências |

| D30 | Dois DOUs, dois índices | Fase 2 (v2) | **a camada do DOU vira `data/dou/` e cada nome diz o GRÃO e o ASSUNTO.** Desfaz duas colisões: `saidas_dou/` (ATOS) x `saidas_dou.csv` (PESSOAS), e `atos_dou.csv` (fonte) x `atos_dou.json` (resumo). Ficam `atos_saida.csv`+`atos_saida/`, `atos_destino.csv`+`atos_destino/`, `por_pessoa.csv`, `varredura.txt` e `public/dou.json`. **O ato de DESTINO ganha índice e cópia arquivada**: a assimetria estava invertida em relação à confiança — motivo é leitura direta do ato (95%) e tinha cópia; destino é inferência e tinha só uma URL. Arquivar não custa requisição nova. São dois índices e não uma coluna `TIPO` porque o resumo do site é DERIVADO do índice de saídas, e um ato de chegada lá dentro viraria PERDA da CGU na primeira derivação que esquecesse de filtrar. `gerar_card_dou.py` vira `resumir_dou.py` (o escopo cresceu na D29 e o nome não). Sai `atos.importar_de_saidas_dou`, que importava zero. Medido no backfill: 130 destinos = **34 atos**, 1,0 MB |

| D31 | Uma regra só para "de quem é o ato" | Fase 2 (v2) | **o casamento ato↔pessoa passa a ter uma implementação, nas duas direções.** Havia duas: `enriquecer_saidas.py` parte da PESSOA e decide lendo o texto (`dou.identidade_no_ato`, D25); o resumo do DOU partia do ATO e decidia pela ficha do índice — nome idêntico e matrícula que não conflitasse. As duas **discordavam num ponto escrito na D25**: em ato da CGU a matrícula divergente REBAIXA, e a regra fraca VETAVA. Como a cópia do ato está em disco, `atos.dono_do_ato` lê o documento e chama a mesma função — sem rede. **Levenshtein entra só para ACHAR candidato**, com teto de 2, e quem decide continua sendo a leitura do ato. Os dois números que fixam o teto foram medidos nesta base: o DOU erra o nome em **2 de 270** atos, sempre por **1** caractere (`KRANZFIELD`/`KRANZFELD`, `PAGLIONE`/`PAGLIONI`); e entre os 2.009 nomes há **zero** pares a distância 1 ou 2, com o primeiro par a 3 sendo `FABIO PEREIRA CARDOSO` x `LUCIO PEREIRA CARDOSO` — duas pessoas. Provado antes de valer: os 277 atos foram levantados com a regra velha e com a nova, e **nenhum trocou de dono** (275 com dono, 2 sem, nos dois); o `dou.json` saiu byte a byte igual. Os dois erros de grafia reais, simulados sem dono, a regra antiga perdia e a nova acha |

| D32 | O SIAPE entra sozinho, e o snapshot é versionado | Fase 2 (v2) | **a rotina mensal inteira passa a rodar no CI** (`atualizar-siape.yml`), o que exigia resolver de onde vem a série. Os snapshots saem de `historico_transparencia_cgu/` (43 colunas, 1,4 MB/mês, fora do Git) para **`data/siape/AAAAMM.csv.gz`**: as **16 colunas** que a derivação lê, comprimidas, **89 KB por competência**, arquivo NOVO a cada mês e nunca reescrito — 4,4 MB a série inteira, ~1 MB/ano. É **menos churn do que o repositório já tinha**: o `dados.csv` sozinho são 400 KB REESCRITOS por competência. **Não dava para processar só o mês novo em memória**: a derivação é sobre a série inteira (primeira aparição, última presença, valor modal do ingresso, e as 6 pessoas que somem e voltam), e uma derivação incremental seria uma SEGUNDA definição de "saiu" — a classe de defeito que a D31 acabou de remover. **O CPF decide a projeção**, não o tamanho: das 27 colunas descartadas uma é o CPF mascarado (`***.930.011-**`), 6 dos 11 dígitos ao lado do nome completo, num repositório público. O bruto (ZIP ~80 MB, CSV ~420 MB) continua efêmero nos dois lados. `atualizar.py --baixar` vira o passo 0; `testar_dou.py` e `testar_publicacao.py` rodam ANTES do crawler; o resumo do pipeline vai para o CORPO DO COMMIT, para os avisos não morrerem num log. Provado: o painel reconstruído das 16 colunas comprimidas saiu **byte a byte igual** ao construído das 43 |
...
- **D27 revista pela D28 num ponto.** Convocação de aprovado deixa de publicar: ela é chamado a comparecer, não entrada em cargo, e três convocações seguidas da mesma prefeitura são o retrato de quem **não** compareceu. Ela continua indo para a pauta, onde vale como pista. O resto da D27 fica de pé.
- **D16 vira insumo de verdade pela primeira vez, mas não é ela (D32).** O que passou a alimentar o CI não é o `historico_mensal.csv` empilhado — é o snapshot POR COMPETÊNCIA, pequeno e append-only. A intuição da D16 (guardar a série) estava certa; a forma (um arquivo único reescrito) é que era errada.
- **D25 passa a valer nas duas direções (D31).** A regra "a matrícula divergente rebaixa, não veta" estava implementada só no caminho pessoa→ato. O caminho ato→pessoa tinha regra própria e a contrariava. Agora há uma função só, e os 63 casos de regressão do `testar_dou.py` cobrem os dois sentidos.
- **D19 sobrevive inteira, e a D30 não a contraria.** A D19 juntou o que estava DUPLICADO — o mesmo ato em duas pastas, com dois JSONs discordando. A D30 separa o que é DIFERENTE: ato de saída da CGU e ato de chegada em outro órgão, que nunca compartilham um `URL_TITLE`.
- **D22 e D24 sobrevivem inteiras no QUE, revistas pela D29 no ONDE.** O que se mescla, as guardas contra contar errado e a precedência CURADORIA > DOU > RANKING continuam idênticas — só deixam de rodar no navegador e passam a rodar na publicação. A justificativa original ("no navegador porque o `construir_painel.py` não roda no CI") continua verdadeira sobre o `construir_painel.py`, e apenas sobre ele.
