...
| `AREA` | `AREA` | `Auditoria e Fiscalização`, `TI`, `Contabilidade Pública e Finanças`, `Correição e Combate à Corrupção` |
...
> **Sem coluna `CARGO`** (**D7** revogou a **D4**): o observatório cobre um cargo só, AFFC. Se o TFFC
> voltar ao escopo algum dia, é aqui que a coluna entra.
...
- [ ] `evasao/types.ts` — hoje tem só 4 linhas (`DadosDestinoEvasao`). Tipar o registro do CSV de verdade (`RegistroAuditor`), em vez do `any[]` usado hoje no `App.tsx`.
- [ ] `evasao/constants.ts`:
  ...
  - adicionar constantes de domínio: `AREAS`, `SITUACOES` (não `CARGOS` — ver D7)
- [ ] Criar `evasao/data/dados.csv` novo com o cabeçalho acima + **~10 linhas fictícias marcadas como EXEMPLO** (coluna `OBSERVACAO` = `EXEMPLO — dado fictício`), cobrindo: `EM EXERCÍCIO`, `EXONERADO`, `DESISTENTE`, `APOSENTADO`, `CADASTRO DE RESERVA` e as 4 áreas AFFC. **Nenhuma linha de TFFC.**
...
- [ ] **Filtro de área** (`App.tsx:1414-1425`) — hoje monta os botões a partir de `FISCALIZAÇÃO / TI / TRIBUTAÇÃO / VETERANO`. Trocar pelas 4 áreas AFFC. Idem `DetailedTableApp.tsx:6,415-424`, que tem `'FISCALIZAÇÃO'` hardcoded como área padrão.
- [ ] **"VETERANO"** — no modelo MG era um valor da coluna `AREA` que distinguia servidores antigos dos aprovados no concurso. Sem coluna `CARGO` (**D7**), o equivalente CGU é a **ausência de `POSICAO_CONCURSO`**. Ajustar os agregadores.
...
- [ ] Carregar a lista de aprovados do **Resultado Final FGV de 13/06/2022** (fonte no CLAUDE.md) para popular o `dados.csv` real — **apenas as listas de AFFC**, descartando as de TFFC (**D7**).
