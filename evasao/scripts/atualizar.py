...
Encadeia o pipeline inteiro:
    ...
    enriquecer_saidas motivo e destino de cada saída nova, no DOU
...
def main() -> int:
    ...
    if not rodar("1/4 Filtrando os snapshots do Portal", ["filtrar_affc.py"]):
        return 1

    if args.concurso and not rodar("Resultado final do concurso (DOU)", ["concurso.py"]):
        return 1

    if not rodar("2/4 Construindo o painel", ["construir_painel.py"]):
        return 1
    ...
    if not rodar("3/4 Buscando motivo e destino no DOU", enriquecer):
        return 1

    if not rodar("4/4 Reconstruindo o painel com o que o DOU achou", ["construir_painel.py"]):
        return 1
